...
function checkDuplicate() {
  ...
  if (itemText === "") {
    alert("Input cannot be empty. Please enter a valid item.");
    return false;
  }
  ...
  if (isDuplicate) {
    alert(
      `"${itemText}" is already on the list. Duplicate entries are not allowed.`
    );
    return false;
  }
...
function renderList() {
  ...
  listArr.forEach((gift, index) => {
    ...
    deleteButton.addEventListener("click", () => handleDeleteItem(index));
